...
pub fn stake_handler(
    ...
    if position.stake_amount == 0 {
        ...
        position.entry_index_bps = u16::try_from(weighted).map_err(|_| EscrowError::MathOverflow)?;
...
fn verify_entry_quote(
    ...
    require!(ed_ix.program_id == ED25519_ID, EscrowError::MissingQuoteSignature);
    ...
    require!(msg_size == QUOTE_MSG_LEN, EscrowError::MalformedQuoteSignature);
    let msg = data
